...
public class TaskWorkspaceTests
{
    [Fact]
    public async Task Workspace_requires_owner_or_valid_membership_even_for_public_tasks()
    {
        ...
        var workspace = WorkspaceTestServices.Create(context);
        Assert.True(await workspace.CanAccessAsync(1, 1));
        Assert.True(await workspace.CanAccessAsync(1, 2));
        Assert.False(await workspace.CanAccessAsync(1, 3));
        Assert.False((await workspace.GetActivitiesAsync(1, 3)).Success);
        Assert.False((await workspace.GetMessagesAsync(1, 3)).Success);
        Assert.False((await workspace.SendMessageAsync(1, 3, "Unauthorized")).Success);
        Assert.True((await workspace.SendMessageAsync(1, 2, "View participant can discuss")).Success);
        (await context.TaskShares.SingleAsync()).Permission = (TaskPermission)0;
        await context.SaveChangesAsync();
        Assert.False(await workspace.CanAccessAsync(1, 2));
    }
    ...
    public async Task Messages_validate_content_and_persist_despite_realtime_failure()
    {
        ...
        var workspace = WorkspaceTestServices.Create(context, failRealtime: true);
        Assert.False((await workspace.SendMessageAsync(1, 1, " \t\n")).Success);
        Assert.False((await workspace.SendMessageAsync(1, 1, new string('a', 4001))).Success);
        var sent = await workspace.SendMessageAsync(1, 1, "Persisted discussion");
        Assert.True(sent.Success);
        using var fresh = database.CreateContext();
        var message = Assert.Single((await WorkspaceTestServices.Create(fresh).GetMessagesAsync(1, 1)).Data);
    ...
    public async Task Activity_writer_defers_save_and_task_creation_records_structured_history()
    {
        ...
        await context.SaveChangesAsync();
        var history = (await WorkspaceTestServices.Create(fresh).GetActivitiesAsync(task.Id, 1)).Data;
